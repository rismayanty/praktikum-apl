...
void insertionSortNamaDescending(member arr[], int n) {
    for (int i = 1; i < n; i++) {
        member key = arr[i];
        int j = i - 1;
...
void bacasemuadata(member members[], int jumlahmember) {
    ...
    if (opsi == 1) {
        insertionSortNamaDescending(temp, jumlahmember);
        cout << "\nData diurutkan: Nama (Z -> A)" << endl;
    } else if (opsi == 2) {
        insertionSortIdAscending(temp, jumlahmember);
        cout << "\nData diurutkan: ID (1 -> 100)" << endl;
    } else if (opsi == 3) {
        quickSortHarga(temp, 0, jumlahmember - 1);
        cout << "\nData diurutkan: Harga (Termurah)" << endl;
    } else {
